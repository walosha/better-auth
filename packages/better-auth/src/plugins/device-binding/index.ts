...
async function defaultDeviceFingerprinting(context: GenericEndpointContext, opts?: { deviceId: string, platform: string}): Promise<string> {
      ...
      registerDeviceWithOTP: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          if (step === "request") {
            ...
            // Set cookies
            if (shouldTrust) {
              await setDeviceBindingCookie(ctx, deviceInfo?.deviceId ?? deviceId,deviceFingerprint);
            }
      ...
      after: [
        {
          ...
          handler: createAuthMiddleware(async (ctx) => {
            ...
            const uniqueDeviceId = ctx.headers?.get("x-device-id") ?? ""
            ...
            const currentFingerprint = await opts.generateDeviceFingerprint(ctx, { deviceId: uniqueDeviceId, platform});
            ...
            if (!trustedDevice) {
              ...
              // Check expiration
              if (trustedDevice?.expiresAt && new Date() > new Date(trustedDevice.expiresAt)) {
                await ctx.context.adapter.update({
                  model: opts.deviceBindingTable,
            ...
            if (opts.requireDeviceVerification) {
              ...
              return ctx.json({
                ...
                deviceId: existingDevice?.deviceId ?? uniqueDeviceId,
